  ...
  return (
    <div className="space-y-3">
      <div className={`rounded-lg px-4 py-3 text-sm font-medium ${allOk ? "bg-emerald-50 text-emerald-700 border border-emerald-200" : "bg-rose-50 text-rose-700 border border-rose-200"}`}>
        {allOk
          ? "Stock sufficient — can ship"
          : `Insufficient stock — ${shortages.length} item${shortages.length > 1 ? "s" : ""} short`}
      </div>
...
// ── Create Form ────────────────────────────────────────────────────────────
function CreateForm({ destinations, onCreated, onCancel }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setError("")
    try {
      await createPlannedShipment({ destination, notes, lines })
      onCreated()
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="space-y-2">
        <label className="text-xs font-semibold text-slate-600">Add SKU Lines</label>
...
// ── Detail Panel ────────────────────────────────────────────────────────────
function DetailPanel({ shipment, onStatusChange, onDelete }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <span className={`inline-flex shrink-0 rounded-full px-2.5 py-1 text-xs font-semibold ${statusStyles[shipment.status]}`}>
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          ...
          <p className="text-xs text-slate-400 mt-0.5">{formatDate(s.created_at)} · {(s.warehouse_planned_shipment_lines || []).length} SKU{(s.warehouse_planned_shipment_lines || []).length !== 1 ? "s" : ""}</p>
        </div>

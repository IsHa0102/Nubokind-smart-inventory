...
function ProductList({ products, onRefresh, addToast, search }) {
  const [addForm, setAddForm] = useState({ name: "", stock: "", low_stock_threshold: "" })
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      setAddForm({ name: "", stock: "", low_stock_threshold: "" })
      onRefresh()
  ...
  return (
    <>
      ...
      <form onSubmit={handleAdd} className="mb-5 rounded-xl border border-slate-200 bg-slate-50 p-4">
        <p className="mb-3 text-xs font-semibold uppercase tracking-wide text-slate-500">Add New Product</p>
      ...
      {/* ── Table ── */}
      {filtered.length === 0 ? (
        <p className="py-8 text-center text-sm text-slate-400">{search ? "No products match your search." : "No products yet."}</p>
      ) : (

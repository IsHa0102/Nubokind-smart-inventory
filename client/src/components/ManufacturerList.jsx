...
import { createManufacturer, deleteManufacturer, updateManufacturer } from "../api/inventoryApi"
import ConfirmModal from "./ConfirmModal"
import EditModal from "./EditModal"

const NAME_FIELDS = [{ key: "name", label: "Manufacturer Name", type: "text", required: true }]

function ManufacturerList({ manufacturers, onRefresh, addToast, search }) {
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      setNewName("")
      onRefresh()
      addToast("Manufacturer added successfully", "success")
    } catch (err) {
      addToast(err?.response?.data?.message || "Failed to add manufacturer", "error")
    } finally {
      setAdding(false)
    }
  }
  ...
  const handleSave = async (values) => {
    setSaving(true)
    try {
      await updateManufacturer(editTarget.id, { name: values.name })
      setEditTarget(null)
      onRefresh()
      addToast("Manufacturer updated successfully", "success")
    } catch (err) {
      addToast(err?.response?.data?.message || "Failed to update manufacturer", "error")
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      onRefresh()
      addToast("Manufacturer deleted successfully", "success")
    } catch (err) {
      setDeleteTarget(null)
      addToast(err?.response?.data?.message || "Failed to delete manufacturer", "error")
    } finally {
      setDeleting(false)
    }
  }
  ...
  return (
    <>
      ...
      <form onSubmit={handleAdd} className="mb-5 rounded-xl border border-slate-200 bg-slate-50 p-4">
        <p className="mb-3 text-xs font-semibold uppercase tracking-wide text-slate-500">Add New Manufacturer</p>
      ...
      {filtered.length === 0 ? (
        <p className="py-8 text-center text-sm text-slate-400">
          {search ? "No manufacturers match your search." : "No manufacturers yet."}
        </p>
      ) : (
      ...
      <EditModal
        isOpen={!!editTarget}
        title={`Edit Manufacturer — ${editTarget?.name ?? ""}`}
        fields={NAME_FIELDS}
